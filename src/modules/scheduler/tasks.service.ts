...
export class TasksService {
  ...
  /**
   * Main scheduled task - Process odds and build bets
   * Runs every 30 minutes by default (configurable via env)
   */
  ...
  async processOdds() {
    ...
    try {
      ...
      // Step 5: Build bet pairs
      this.logger.logProcessing('TasksService', 'Step 5/6: Building bet pairs');
      const pairs = await this.pairBuilderService.buildPairs();

      if (pairs.length === 0) {
        this.logger.logWarning('TasksService', 'No pairs could be built');
      } else {
        // Step 6: Save bets
        this.logger.logProcessing('TasksService', 'Step 6/6: Saving bet pairs');
        const savedBets = await this.betsService.createMany(pairs);
        this.logger.logSuccess('TasksService', `Created ${savedBets.length} bet pairs`);
      }
      ...
      this.logger.log(`Bet pairs created: ${pairs.length}`, 'TasksService');
  ...
  async healthCheck() {
    ...
    const stats = await this.betsService.getStatistics();
    const pairingStats = await this.pairBuilderService.getPairingStats();

    this.logger.log(`Total bets: ${stats.total}`, 'TasksService');
    this.logger.log(`Pending opportunities: ${pairingStats.pendingOpportunities}`, 'TasksService');
    this.logger.log(`Total profit: R$ ${stats.totalProfit.toFixed(2)}`, 'TasksService');
  }
}
